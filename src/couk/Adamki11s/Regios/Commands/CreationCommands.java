...
public class CreationCommands {
	...
	public static void cancelSelection(Player p){
		if(point1.containsKey(p)){ point1.remove(p); }
		if(point2.containsKey(p)){ point2.remove(p); }
		if(mod1.containsKey(p)){ mod1.remove(p); }
		if(mod2.containsKey(p)){ mod2.remove(p); }
	}
